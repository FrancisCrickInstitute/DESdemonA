...
```bash
ts desdemona babsfile=/location/of/.babs
``` 

If you're in a directory with a `.babs` file, then there is no need to
set that explicity, so `ts desdemona` will suffice.

This will create all the required files in the current directory.
