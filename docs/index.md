...
Currently, I maintain  makefile for initiating new projects. If you're
in the directory where you want to store your analysis files then you
can get a copy of the necessary makefiles by

```bash
cp -n /camp/stp/babs/working/kellyg/templates/babs/* .
``` 

and then generate the skeleton scripts by

```bash
make init
```

THis will create all the required files in the ``project_folder``
directory. You may notice that it contains a ``DESCRIPTION`` file -
...
The `DESCRIPTION` file contains a generic project title, but if you're
